"""JEPA training loop for UrbanJEPA.
...
text_emb key in dataset.pt: [N, text_dim] pre-computed sentence embeddings.
Falls back to zeros when the key is absent (backward compat with datasets built
before text conditioning was introduced).
"""
...
class UrbanWindowDataset(Dataset):
    """Wraps a split dict from dataset.pt.

    Synthesises text_emb as zeros if the key is absent (pre-text datasets).
    text_dim is inferred from the split or falls back to the Qwen3-Embedding-0.6B default.
    """
    _state_keys = ("values_ctx", "masks_ctx", "quality_ctx", "static", "values_tgt", "masks_tgt")

    def __init__(self, split: dict[str, torch.Tensor], text_dim: int = 1024) -> None:
        self.data = {k: split[k] for k in self._state_keys}
        N = len(self.data["values_ctx"])
        self.data["text_emb"] = split.get(
            "text_emb", torch.zeros(N, text_dim, dtype=torch.float32)
        )
    ...
    def __getitem__(self, i: int) -> tuple[torch.Tensor, ...]:
        return (
            self.data["values_ctx"][i],
            self.data["masks_ctx"][i],
            self.data["quality_ctx"][i],
            self.data["text_emb"][i],
            self.data["static"][i],
            self.data["values_tgt"][i],
            self.data["masks_tgt"][i],
        )
...
class JEPATrainer:
    ...
    def train_epoch(self) -> float:
        ...
        for batch in self.loader:
            v_ctx, m_ctx, q_ctx, text_emb, static, v_tgt, m_tgt = self._to(*batch)
            z_pred, z_tgt, y_hat = self.model(
                v_ctx, m_ctx.bool(), q_ctx, text_emb, static,
                v_tgt, m_tgt.bool(),
            )
    ...
    def train(self, epochs: int) -> None:
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.opt, mode="min", factor=0.5, patience=5, min_lr=1e-5
        )
        for epoch in range(epochs):
            ...
            logger.info(f"epoch={epoch + 1}/{epochs} loss={avg:.5f} lr={lr:.2e}")
        self.writer.close()
...
def main() -> None:
    ...
    text_dim = int(cfg.get("text_dim", 1024))
    blob     = torch.load(args.dataset, map_location="cpu", weights_only=False)
    ds       = UrbanWindowDataset(blob["train"], text_dim=text_dim)
